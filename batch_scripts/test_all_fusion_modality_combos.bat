@echo off
@break off
@title Run test on every 3D-Densenet fusion branch modality combination
@color 0a
@cls
...
set test_name=test_fusion_combos
...
for /l %%x in (1, 1, %end%) do (
    if %%x LSS %max_num_channels% (
        ...
        python main.py --experiment-name !experiment! --experiment-number %%x --output-path %working_dir% --save-experiment-path "%save_experiment_path%!experiment!.json" --band-reduction-method pca --n-components %%x %flags% 
...
echo "test_all_fusion_modality_combos.bat script completed!"