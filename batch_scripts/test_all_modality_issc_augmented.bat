@echo off
@break off
@title Run ISSC band selection tests on all modalities with data augmentation
@color 0a
@cls
...
set test_name=test_all_modality_issc_augmented
...
set flags=--cuda 0 --dataset grss_dfc_2018 --skip-data-postprocessing --model-id 3d-densenet-modified --center-pixel --split-mode fixed --optimizer nadam --flip-augmentation --radiation-augmentation --mixture-augmentation %training_params% %hs_flags% %lidar_ms_flags% %lidar_ndsm_flags% %vhr_rgb_flags%
...
echo "test_all_modality_issc_augmented.bat script completed!"